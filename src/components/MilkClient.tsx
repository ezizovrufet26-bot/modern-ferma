    ...
    <div className="p-8 max-w-7xl mx-auto space-y-10 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-4xl font-black text-gray-900 tracking-tight flex items-center gap-4">
             ...
             Süd <span className="text-blue-600">İstehsalı</span>
          </h1>
          <p className="text-gray-500 mt-3 font-medium flex items-center gap-2">
            <Beaker className="w-4 h-4 text-blue-500" /> Günlük süd verimi və analitik hesabatlar
          </p>
        </div>
        <button 
        ...
        >
          <Plus className="w-6 h-6" /> Yeni Süd Qeydi
        </button>
      </header>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="glass-panel rounded-[40px] p-8 shadow-2xl shadow-blue-500/5 relative overflow-hidden group border border-white/50">
          ...
          <p className="text-gray-400 font-black text-[10px] uppercase tracking-[0.2em] mb-2">Bugünkü İstehsal</p>
          <h3 className="text-4xl font-black text-gray-900 tracking-tighter">{totalToday.toLocaleString()} <span className="text-lg text-gray-400 ml-1">Litr</span></h3>
        ...
        <div className="glass-panel rounded-[40px] p-8 shadow-2xl shadow-emerald-500/5 relative overflow-hidden group border border-white/50">
          ...
          <h3 className="text-4xl font-black text-gray-900 tracking-tighter">
            {(last7Days.reduce((acc, d) => acc + d.amount, 0) / 7).toFixed(1)} <span className="text-lg text-gray-400 ml-1">Litr</span>
          </h3>
      ...
      <section className="glass-panel rounded-[48px] overflow-hidden shadow-2xl shadow-blue-500/5 border border-white/50 bg-white/30 backdrop-blur-md">
         <div className="p-10 border-b border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
               <h2 className="text-2xl font-black text-gray-900 tracking-tight">Süd Tarixçəsi</h2>
               ...
               <table className="w-full text-left border-separate border-spacing-y-3">
                  <thead>
                    <tr className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">
                       <th className="px-8 py-4">Heyvan / Bırka</th>
                       <th className="px-8 py-4">Tarix</th>
                       <th className="px-8 py-4 text-center">Səhər (L)</th>
                       <th className="px-8 py-4 text-center">Axşam (L)</th>
                       <th className="px-8 py-4 text-center">Cəmi (L)</th>
          ...
          <form action={async (formData) => {
            ...
            <div className="flex justify-between items-center">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-black text-gray-900 text-xl md:text-2xl tracking-tight">{editingRecord ? 'Düzəliş Et' : 'Yeni Süd Qeydi'}</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
              {!editingRecord && (
                <div className="col-span-full space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Heyvan Seçin</label>
              ...
              <div className="space-y-2 md:space-y-3">
                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Səhər Verimi (L)</label>
              ...
              <div className="space-y-2 md:space-y-3">
                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Axşam Verimi (L)</label>
            ...
            <div className="flex gap-4 md:gap-6 pt-2 md:pt-4">
              <button type="submit" className="flex-1 bg-blue-600 text-white py-4 md:py-6 rounded-2xl md:rounded-[32px] text-base md:text-lg font-black hover:bg-blue-700 transition-all shadow-2xl shadow-blue-600/30">Yadda Saxla</button>
              <button type="button" onClick={() => setShowModal(false)} className="px-6 md:px-12 py-4 md:py-6 bg-gray-50 text-gray-500 rounded-2xl md:rounded-[32px] text-base md:text-lg font-black hover:bg-gray-100 transition-all border border-gray-100">Ləğv Et</button>
            </div>
          </form>
        </div>
      )}
